...
class Solution{
    ...
    Node* solve(int preorder[], int maxi, int &index, int size) {
        if(index >= size)
            return NULL;
        
        if(preorder[index] > maxi)
            return NULL;
        
        Node* root = newNode(preorder[index++]);
        root->left = solve(preorder, root->data, index, size);
        root->right = solve(preorder, maxi, index, size);
        return root;
    }
...
public:
    ...
    {
        int i = 0;
        return solve(pre, INT_MAX, i, size);
    }
};
